...
def _build_context(ticker: str) -> list:
    ...
    try:
        ...
        if brief and brief.get("net_m") is not None:
            ...
            lines.append(
                f"  {sign} Insider 90d: net ${net:+.0f}M ({brief.get('n_buys', 0)}B/{brief.get('n_sells', 0)}S)"
            )
...
def _format_alert(ticker: str, level: str, thesis, current_price: float) -> str:
    ...
    try:
        ...
        if pos and pos["qty"] > 0:
            ...
            generic = {
                "partial": "sell 1/3 of position",
                "full": "sell remaining position",
                "stop": "EXIT — invalidation breached",
            }.get(level, "review")
...
def _maybe_update_clv(cx, thesis, current_price: float) -> None:
    ...
    for d, col_p, col_c in [(7, "price_7d", "clv_7d"), (30, "price_30d", "clv_30d"), (90, "price_90d", "clv_90d")]:
        if d <= days_since and col_c in keys and thesis[col_c] is None:
            ...
            cx.execute(f"UPDATE theses SET {col_p} = ?, {col_c} = ? WHERE id = ?", (current_price, clv, thesis["id"]))
            ...
            try:
                ...
                for level in crossings:
                    ...
                    try:
                        ...
                        cx.execute(f"UPDATE theses SET triggered_{level}_at = ? WHERE id = ?", (now_iso, t["id"]))
                ...
                cx.execute("UPDATE theses SET last_price=?, last_price_at=? WHERE id=?", (p, now_iso, t["id"]))
...
def record_override(ticker: str, level: str, reason: str) -> int:
    with db() as cx:
        _ensure_overrides_table(cx)
        r = cx.execute(
            "SELECT id FROM theses WHERE ticker=? AND status='active' ORDER BY id DESC LIMIT 1", (ticker.upper(),)
        ).fetchone()
...
def list_overrides(limit: int = 20) -> list:
    with db() as cx:
        _ensure_overrides_table(cx)
        rows = cx.execute(
            "SELECT id, ticker, thesis_id, level, reason, created_at FROM overrides ORDER BY id DESC LIMIT ?", (limit,)
        ).fetchall()
