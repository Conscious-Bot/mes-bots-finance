...
def sentiment_polarity(signal):
    ...
    if isinstance(s, (int, float)):
        ...
        return "neutral"
    s = str(s).lower().strip()
    if s in ("positive", "bullish", "buy", "+", "pos"):
        return "positive"
    if s in ("negative", "bearish", "sell", "-", "neg"):
        return "negative"
    return "neutral"
...
def derive_signal_type(signal):
    ...
    if any(k in text for k in ("insider", "form 4", "10b5-1")):
        return "insider"
    if any(k in text for k in ("guidance ", "outlook ", "forecast ")):
        return "guidance"
    if any(k in text for k in ("earnings beat", "earnings miss", "eps beat")):
        return "earnings"
    if any(k in text for k in ("upgrade", "downgrade", "price target")):
        return "analyst"
    if any(k in text for k in ("fomc", "powell", "rate cut", "rate hike", "cpi report")):
        return "macro"
    if any(k in text for k in ("capex", "supply chain", "supplier")):
        return "industry"
    return "news"
...
def _ticker_importance(ticker, watchlist=None):
    ...
    if t in MEGA_CAP:
        return 1.0
    if watchlist and t in {w.upper() for w in watchlist}:
        return 0.75
    if t in LARGE_CAP:
        return 0.65
    return 0.4
...
def score_cross_confirmation(signal, recent_signals):
    ...
    for other in recent_signals or []:
        ...
        if not (tm or nm):
            continue
        if sentiment_polarity(other) != target_polarity:
            continue
...
def score_regime_relevance(signal, regime_info):
    ...
    if sig_type == "insider":
        if polarity == "positive":
            ...
            if credit_class == "TIGHT":
                base = 0.80
            elif credit_class in ("STRESSED", "CRISIS"):
                base = 0.50
            else:
                base = 0.60
    elif sig_type in ("earnings", "guidance"):
        base = 0.85
    elif sig_type == "macro":
        base = 0.75
    elif sig_type == "industry":
        base = 0.70
    elif sig_type == "analyst":
        base = 0.50
    elif sig_type == "crypto":
        base = 0.55
    elif sig_type == "news":
        base = 0.40
...
def score_materiality(signal, recent_signals_24h=None, recent_signals_72h=None, regime_info=None, watchlist=None):
    ...
    return {
        ...
        "quality": quality,
        "novelty": novelty,
        "cross_confirmation": cross_conf,
        "market_impact": market_impact,
        "regime_relevance": regime_rel,
        "noise": noise,
        "_derived": {
            "tickers": extract_tickers(signal),
