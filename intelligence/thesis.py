...
def add_thesis(ticker, direction, horizon, conviction,
               key_drivers, invalidation_triggers,
               entry_price, target_partial=None, target_full=None,
               triggers_profit_take=None, target_price=None, notes=None):
    ...
    thesis_id = storage.insert_thesis(
        ticker=ticker, direction=direction, horizon=horizon, conviction=conviction,
        key_drivers=key_drivers, invalidation_triggers=invalidation_triggers,
        entry_price=entry_price, target_price=target_price,
        target_partial=target_partial, target_full=target_full,
        triggers_profit_take=triggers_profit_take, notes=notes,
    )
...
def format_thesis_card(thesis):
    ...
    status_icon = {'active': 'ACTIVE', 'invalidated': 'INVALIDATED',
                   'realized': 'REALIZED', 'stale': 'STALE'}.get(thesis.get('status'), '?')
...
def check_exit_request(ticker, current_price=None):
    ...
    if not thesis:
        return {
            'thesis_found': False, 'status': 'no_thesis',
    ...
    if triggers_met:
        return {
            'thesis_found': True, 'thesis': thesis, 'status': 'trigger_met',
    ...
    return {
        'thesis_found': True, 'thesis': thesis, 'status': 'no_trigger',
        'triggers_met': [], 'message': "\n".join(parts)
    }
