        ...
        try:
            ...
            for f in filings:
                ...
                row_id = storage.log_8k_filing(
                    ticker=tk, cik=f['cik'], accession=f['accession'],
                    filed_at=f['filed_at'], items_raw=f['items_raw'],
                    item_codes=f['item_codes'], severity=severity,
                    severity_reason=reason, filing_url=f['url']
                )
                if row_id:
                    log.info(f"8-K logged: {tk} {f['filed_at']} {f['items_raw']} [{severity}] id={row_id}")
                    new_logged.append({
                        'id': row_id, 'ticker': tk, **f,
...
def format_8k_alert(row):
    ...
    icon = {'catastrophic': 'CATASTROPHIC', 'high': 'HIGH',
            'medium': 'MEDIUM', 'low': 'LOW'}.get(row['severity'], '?')
