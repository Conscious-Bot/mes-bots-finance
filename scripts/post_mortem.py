...
def report(days: int = 10, out_path: str | None = None):
    ...
    def resolve_source(v):
        if v is None:
            return "?"
        if isinstance(v, int) and v in src_map:
            return src_map[v]
        return str(v)
    ...
    if sc:
        ...
        if date_col:
            recent = cx.execute(f"SELECT * FROM signals WHERE {date_col} >= ? ORDER BY id DESC", (cutoff,)).fetchall()
        else:
    ...
    if cols(cx, "predictions"):
        ...
        if resolved > 0:
            ...
            avg = (
                cx.execute("SELECT AVG(return_pct) AS r FROM predictions WHERE outcome IS NOT NULL").fetchone()["r"]
                or 0
            )
            ...
            if rec:
                ...
                for r in rec:
                    p(
                        f"    [{r['id']:3d}] {r['ticker']:6s} {r['direction']:5s} "
    ...
    events = cx.execute(
        "SELECT date, event_type, ticker, description FROM events WHERE date BETWEEN ? AND ? ORDER BY date",
        (today, end30),
    ).fetchall()
    ...
    if cols(cx, "sources"):
        ...
        if srcs:
            ...
            for s in srcs:
                ...
                p(
                    f"  {(s['name'] or '?')[:24]:24s} cred={s['credibility']:.3f} "
