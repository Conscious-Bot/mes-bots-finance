...
TARGETS = [
    ...
    ("7011.T",    "TR", "power",         3500, 5.4, "MHI turbines + nuclear + defense (incl topup planned W1)", "tier_s", "executed", 1),
    ...
    ("6273.T",    "TR",  "moonshots",    1750, 2.7, "SMC robotics actuators (range 1500-2000)", "watchlist_conditional", "watchlist_conditional", None),
    ("8035.T",    "TR",  "core_ai",      1750, 2.7, "Tokyo Electron WFE chokepoint (range 1500-2000)", "watchlist_conditional", "watchlist_conditional", None),
    ("ASM.AS",    "PEA", "core_ai",      1250, 1.9, "ASM Intl complete Dutch trio (range 1000-1500)", "watchlist_conditional", "watchlist_conditional", None),
        ...
        for ticker, account, bucket, target_eur, weight_pct, narrative, priority, status, phase_week in TARGETS:
            ...
            cx.execute(
                """INSERT INTO portfolio_targets
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (ticker, account, bucket, target_eur, weight_pct, narrative,
                 priority, status, phase_week, SOURCE_DOC, tid),
            )
            inserted += 1
