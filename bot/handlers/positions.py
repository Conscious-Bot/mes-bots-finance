...
async def cmd_trade(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Usage:
      /trade buy TICKER QTY PRICE [reasoning]   → delegates to cmd_position_buy
      /trade sell TICKER QTY PRICE [reasoning]  → delegates to cmd_position_sell

    Backward-compat aliases preserved 1 release cycle:
      /position_buy [...]
      /position_sell [...]

    Implementation: text-rewrite wrapper. Preserves 100% of cmd_position_buy
    and cmd_position_sell internals including:
    - B2 risk_engine.validate() gate
    - B5 journal_mod log_decision chain (KPI #5 critical)
    - bias_tagger auto-tagging post-decision
    - position_events full audit trail
    """
    ...
    # Text rewrite: transform /trade buy NVDA 10 450 → /position_buy NVDA 10 450
    # This preserves cmd_position_buy/sell parsing without touching their internals.
    target_cmd = f"/position_{action}"
    rest = " ".join(args[1:])
    original_text = update.message.text
    update.message.text = f"{target_cmd} {rest}".rstrip()
    try:
        if action == "buy":
            await cmd_position_buy(update, ctx)
        else:
            await cmd_position_sell(update, ctx)
    finally:
        # Always restore original text, even on exception
