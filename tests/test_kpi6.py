...
def test_compute_kpi6_no_positions(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(pm, "compute_portfolio_return_eur", lambda: None)
    r = pm.compute_kpi6()
...
def test_compute_kpi6_benchmark_fetch_fail(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(pm, "compute_portfolio_return_eur", lambda: _mock_pf(400, 10.0))
    monkeypatch.setattr(pm, "fetch_benchmark_return_eur", lambda tk, days: None)
    r = pm.compute_kpi6()
    assert "INSUFFICIENT_BENCHMARK" in r["status"]


def test_compute_kpi6_green_all_above(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(pm, "compute_portfolio_return_eur", lambda: _mock_pf(400, 15.0))
    monkeypatch.setattr(pm, "fetch_benchmark_return_eur", lambda tk, days: 10.0)
    r = pm.compute_kpi6()
    assert "✅" in r["status"]
    assert "all 3" in r["status"]
    for tk in ("SPY", "QQQ", "SMH"):
        assert f"{tk}-eur" in r["current"]


def test_compute_kpi6_yellow_one_breach_smh(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(pm, "compute_portfolio_return_eur", lambda: _mock_pf(400, 5.0))
    returns = {"SPY": 5.0, "QQQ": 5.0, "SMH": 11.0}
    monkeypatch.setattr(pm, "fetch_benchmark_return_eur", lambda tk, days: returns[tk])
    r = pm.compute_kpi6()
...
def test_compute_kpi6_red_majority_breach(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(pm, "compute_portfolio_return_eur", lambda: _mock_pf(400, 0.0))
    monkeypatch.setattr(pm, "fetch_benchmark_return_eur", lambda tk, days: 10.0)
    r = pm.compute_kpi6()
    assert "🚨" in r["status"]
...
def test_compute_kpi6_insufficient_days(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(pm, "compute_portfolio_return_eur", lambda: _mock_pf(100, 5.0))
    monkeypatch.setattr(pm, "fetch_benchmark_return_eur", lambda tk, days: 5.0)
    r = pm.compute_kpi6()
