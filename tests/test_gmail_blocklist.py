...
@pytest.mark.parametrize(
    ...
    [
        ...
        "Hedgeye <hedgeye@hedgeye.com>",                # interrupteur ① EN ATTENTE
        "The Defiant <no-reply@mail.thedefiant.io>",    # interrupteur ② EN ATTENTE
        "",                                             # vide = pas bloqué
    ],
)
def test_valuable_and_pending_sources_pass(from_addr):
    assert not _is_blocked_sender(from_addr)


def test_blocklist_is_exactly_the_audited_eight():
    """Verrou : la liste ne grossit pas sans passer par ce test (décision consciente)."""
    assert len(_BLOCKED_SENDERS) == 8
