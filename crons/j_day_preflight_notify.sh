...
if [ -f "$PROJECT_DIR/.env" ]; then
    ...
    if [ -n "$TOKEN" ] && [ -n "$CHAT" ]; then
        MSG="J-1 PREFLIGHT (J-day demain 2026-06-10). Verifier maintenant : (1) Mac sera awake demain 09:30 et 14:00 (laptop branche + lid ouvert OU caffeinate). (2) HEALTHCHECKS_J_DAY_URL configure dans .env, check createe sur healthchecks.io avec cron 30 9 10 6 *, grace 4h. (3) Smoke test : bash crons/j_day_watcher.sh en simulant J_DAY=today + creer fake snapshot data/track_record/snapshots/2026-06.json (cleanup apres). (4) Verifier j_day_batch_close_job registered : grep j_day_batch bot.log OU /jobs Telegram. (5) Brier reading contract committed dans docs/."
