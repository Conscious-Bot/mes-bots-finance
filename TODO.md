...
### 🔍 Audit 2e passe (29/05 fin de nuit) — F9-F13, tie-back lucide

User a poursuivi l'audit après le premier round. Sa conclusion explicite : **"ce n'est plus N bugs, c'est une seule maladie qui produit N symptômes. Le motif est invariable : le système calcule beaucoup, ne réconcilie rien, ne valide rien contre la réalité."** À ce stade, collecter plus de findings = rendement décroissant. Le move productif = attaquer les 2 racines, pas patcher symptôme par symptôme.

**Décision** : aucun fix de surface sur F9-F13. Documenter, puis sprint dédié migration lecteurs vers book canonique + audit des inputs.

**F9 — Deux systèmes de classification en parallèle.** La vue Thèses range par thème-secteur ("Foundry & logique", "Connectivité & optique"), la vue Paris range par facteur-macro ("AI capex", "Consumer cyclical"). Pour la même ligne :
- Mitsubishi Heavy = "Défense" (Thèses) / "Industrial reshoring" (Paris) / "turbines gaz + PPA hyperscaler" (fiche). **Trois classifications.**
- STM = "Foundry & logique" / "Consumer cyclical" / "IDM SiC". **Trois aussi.**
- Schneider = "Power & électrification" dans une vue, compté dans AI capex dans une autre.

Conséquence concrète : MHI seul fait bouger la décorrélation de ~5pp selon qu'il est "défense" ou "power-for-AI". **Le ballast change selon la vue lue.** → **Racine #1 driver non canonique**.

**F10 — Stops décrochés du cadre conviction/fade, outliers dangereux.**
- Astera (fragile, valo > bull) → stop **−51%** pour +2% de hausse. Le système te laisserait descendre de moitié sur un nom cassé.
- SK Hynix (qu'on sort) → −43%. Micron → −39%.
- Synopsys (durable) → −20%.

Aucune relation cohérente. Stops semblent générés mécaniquement (multiple fixe sur l'entrée), pas dérivés de la thèse. **Régression du Day 5 : verdict /asymmetry auto-dérivés du cadre = biais de confirmation tautologique. Elle est revenue par la porte des stops.** → **Racine #1 inputs non audités (stop_price)**.

**F11 — La même ligne, poids et verdicts opposés selon la vue.**
- AMD : "✓ au poids" à 3.4% (Thèses) / "sous-pondéré, renforcer" à 1.4% (calibrage) / "orphan, too priced, candidat coupe" (fiche). **Trois poids, trois verdicts sur une ligne.**
- ASML 7.9% vs 5.7%. Shin-Etsu 8.9% vs 10.5%.

Ce ne sont pas que les cibles qui divergent — **les poids ACTUELS divergent** entre vues. Deux lectures du même book affichent une réalité différente. → **Racine #1, le pire symptôme : pas un seul calculateur de poids**.

**F12 — "En gain" et "à risque" se contredisent sur les mêmes noms.** Safran (+10.7%), AMD (+15.1%), Thales (+8%), MP sont à la fois dans la stat-confort "18/27 en gain" ET flaggés "à risque". Deux signaux de santé OPPOSÉS sur la même ligne. → **Racine #1 + #4 : aucune source unique d'état de position**.

**F13 — "Proche de la cible" présenté comme une réussite alors que c'est un signal de danger.** Le seul nom "proche de la cible" est Astera (+2%) — pire couple risque/rendement du book, pas une victoire. Atteindre une cible (mécanique) est affiché neutre/positif quand ça veut dire "surévalué, prends ton profit". → **Racine #4 métrique calibrée pour le confort**.

### Tie-back du user (à graver)

> #1, #3, #4 = pas de book/driver canonique. #2, #5 = paramètres (stops, cibles) générés mécaniquement et jamais validés contre les fondamentaux. **Le motif est invariable : le système calcule beaucoup, ne réconcilie rien, ne valide rien contre la réalité.**

> **Une fois les 2 racines attaquées, 90% de cette liste disparaît d'un coup.**

### Sprint suivant — Migration lecteurs vers book canonique (racine #1 vraie attaque)

Cadre : `shared/book.py` existe depuis ce soir (commit `2534af1`) **mais aucun lecteur n'y est branché**. C'est de la dette architecturale tant que ce n'est pas câblé. Le vrai fix de F9-F13 = migrer les lecteurs principaux.

**Sprint structuré** :

1. **Étendre `BookLine`** :
   - `weight_market_eur` (déjà calculé via `current_eur`)
   - `pct_of_book` (current_eur / total_market)
   - `driver_canonical` (un seul label par position, source = `canonical_perimeter.driver`)
   - `theme_thesis` (la classification "thème" pour les vues Thèses)
   - `macro_factor` (déjà via `ticker_axes`)
   - `conviction` + `fade_rate_score` + `stop_price` + `target_full` (joints avec theses + ticker_meta)

2. **Tagger chaque lecteur actuel** dans render.py + intelligence/*.py qui lit `positions` via raw SQL ou `_positions()`. Identifier ceux qui calculent leur propre poids. Lister les ~15 sites.

3. **Migrer un lecteur à la fois**, chaîné par gates :
   - `_compute_factor_exposures` → `book.get_canonical_book()` (test : exposition AI broad reste 77.0%)
   - `_compute_decorrelation_star` → idem (test : ballast 13.8% conservé)
   - `_compute_quality_T1_plus` → idem (test : Solidité 58.9%)
   - `_compute_mauboussin_sizing` → idem (réutilise weight_market_eur unifié)
   - Panels render.py : `_positions()` remplacé par `get_canonical_book()` partout

4. **Côté thèses** : un seul `theme_thesis` ajouté à chaque BookLine via canonical_perimeter.driver. Plus de classification triple.

5. **Audit des inputs** (passe parallèle) :
   - Pour chaque thèse active : valider `stop_price` cohérent avec fade_rate_score (haut fade → stop plus serré, pas mécanique sur entrée)
   - Pour chaque ticker : valider `conviction` cohérente avec canonical solidite (Incontournable c5 OK, Fragile c4 = signal d'audit)
   - Pour chaque ligne "+gain ET à risque" : forcer une revisit thèse (refonte du flag at_risk pour intégrer le pnl matériel)

6. **Sortir le faux verdict "proche de la cible"** : renommer en "cible bientôt atteinte → prends ton profit" si gain > 80% du chemin entry→target.

Estimation : ~6-10h de focus, idéalement sur 1-2 jours dédiés. **Pas faisable ce soir**. Le bon move = stop, dormir, attaquer frais demain ou après-demain avec ce sprint comme objectif unique de la session.
