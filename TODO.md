...
### 📅 LE MOIS DE SIMPLIFICATION (07/08 → ~07/09) — « déficit d'exposition, pas d'intelligence »
**Critère unique (miroir V2)** : *existe-t-il une décision importante où cette capacité n'est jamais consultée ?*
**Règle** : on ne supprime pas des widgets — on supprime des CHEMINS (position→CF en 1, pas en 4).
**Baseline mesurée 07/08** : Tier-S = distance ∄ sur le dashboard (0 occurrence render.py) ; bot = pull-only.
**Tiers d'excavation (ROI décisionnel)** :
- **S (inévitables)** : ① dernier contrefactuel résolu SUR chaque card (« hold aurait fait +8,4 % ») ·
  ② historique biais du ticker SUR la card (« 17 décisions : lock_in 3, fomo 2 ») ·
  ③ situations similaires Voyage SUR la card (« 2019 : vendu trop tôt »)
- **A** : open_questions qui VIEILLISSENT (âge affiché) · kill criteria qui ATTAQUENT (chaque digest : « quels KC ont bougé ? » — le gate en fait la moitié)
- **B** : events avec CONFIANCE affichée (06/08 · IR · 100 % vs 06/08? · Twitter · 35 % — la leçon Ajinomoto)
**Semaines** :
- **S1** : résoudre Ajinomoto (P2 lundi 10/08 · P1 lecture O.) · SUPPRESSION DE NAVIGATION (clics, pas widgets — compter chaque clic économisé) · rien construire
- **S2** : les CARDS deviennent le centre — 4 réponses (pourquoi je possède · pourquoi je vendrais · qu'est-ce qui invalide · que surveiller aujourd'hui) + les 3 lignes Tier-S (données DÉJÀ calculées, surfaces existantes = autorisé par la lecture du gel « améliorer une carte existante »)
- **S3** : 2-3 pilotes · observer OÙ ILS CHERCHENT (pas ce qu'ils demandent) · zéro correction
- **S4** : élagage final — tout ce qui n'a été utilisé ni par O. ni par les pilotes disparaît
**Compteur à chaque close** : `Ajouts: 0 (capacités/surfaces) · Suppressions: X (cible 100+) · Expositions: Y (données existantes rendues visibles — comptées à part, bornées par le miroir V2)`
...
**Refresh** : 01 juil 2026 close (2) **MARATHON POST-AUDIT** : 11 commits (`a8f169e`→`6bc392a`). Deploy VM (bot rattrapé 12 commits) · package trial sendable + hygiène secrets (6 sortis du repo) · bruit Telegram coupé (anniversaires 26→1 digest, fork persistant-seulement) · Voyage index 5→447 (batché+incrémental+auto-refresh sync, archi Mac-authoritative) · Obsidian A (AMZN 254→5) + E (liens fantômes) + O4 (fail-loud) · **CI VERTE** (allowlist import_book + kill_switch live_data) · kill-criteria fail-closed prix absent (D). **Audit 5 fail-silents : 4/5 traités** (A/D/E ✅, B écarté transitoire, C reste = chantier equity curve). Mac==VM==`6bc392a`. Détail : SESSION_STATE close 2026-07-01 (2). — *Refresh préc. 30/06 : solidification + audit, cf ci-dessous.*
**Refresh (30/06)** : close **SOLIDIFICATION + AUDIT 3 DOMAINES**. 6 commits (`49c98eb`→`45d05da`) : serve résilient (bind-first+launchd KeepAlive), 2 fail-silents tués (`triggered_*_at` révision-reset, `size_recommend` mauvaise table), bandeau daily% honnête + cartes NEAR nominatives, migration 0064 (clamp dust qty vue positions). Data VM : 20 flags triggered orphelins nettoyés, AMD qty→0, Voyage AI câblée (index 5/422 rate-limited). **Audit code+dashboard+Obsidian** → 5 fail-silents HARD actifs (cf ÉTAT SYSTÈME + P0). **⚠️ deploy VM en attente** (bot 12 commits en retard, fast-forward propre). — *Refresh précédent 27/06 : cure taxonomie 5 sources → 1, cf ci-dessous.*
...
### 🔴 P0 AUDIT — 5 fail-silents (01/07 : 4/5 traités)
- ~~**A** aliases churn (`split(",")` → AMZN 235)~~ — **✅ RÉSOLU 01/07** (`792d2d4`) parser quote-aware + dédup, AMZN 254→5 stable.
- ~~**B** `_perf_dwm` vs `_dp_pct` divergent~~ — **❌ ÉCARTÉ 01/07** : vérifié empiriquement, concordent (transitoire cache, pas structurel). Non-bug. Note : peuvent diverger un instant, ne pas "fixer".
- **C** Dashboard equity/CAGR/Sharpe/maxDD **biaisés** (look-ahead qty + survivorship `status='open'` + multi-devise sans FX, `render.py:995-1045`). CONFIRMÉ réel. **SEUL RESTE** — chantier : reconstruire du **ledger transactions** (qty-à-la-date + positions fermées + FX historique). Stratégique (crédibilité track record proof-of-value). *(Mac, chantier moyen)*
- ~~**D** `kill_criteria_monitor` prix=0 → faux KILL~~ — **✅ RÉSOLU 01/07** (`6bc392a`) fail-closed : prix absent → None → skip éval.
- ~~**E** `mirror_transactions` liens `[[fantômes]]`~~ — **✅ RÉSOLU 01/07** (`792d2d4`) resolve→None→texte nu.
