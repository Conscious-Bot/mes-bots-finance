...
- **Jalon A3 (post-10/06) — Wire sourcing existant** : pas "acheter du nouveau". Diagnostic 30/05 itération 4 : EDGAR + 8-K déjà ingérés (`filings_8k_log` 43 rows, `insider_snapshots` 378 rows) mais coulent dans des tables parallèles, jamais wirés au pipeline `signals → V2 → predictions`. Effort révisé : 1-2h pour wire, **+ décision masquée à trancher en session fraîche : backfill historique 421 lignes ou forward-only ?** Backfill = stale-dated predictions = nouvel artefact temporel cousin du cluster horizon=30. À ne pas trancher au commit #16. DoD : voir une vraie 8-K severity=high traverser V2 et sortir un call ≥ 0.65 avec evidence=strong. Sans cette vérif, hypothèse "wire débloque diversité" reste un "devrait", pas un "j'ai vu".
