...
- **Jalon A3 (post-10/06) — Wire sourcing existant** : pas "acheter du nouveau". Diagnostic 30/05 itérations 4-5 : EDGAR + 8-K déjà ingérés (`filings_8k_log` 43 rows, `insider_snapshots` 378 rows) mais (a) coulent dans des tables parallèles, jamais wirés au pipeline `signals → V2 → predictions`, ET (b) `filing_url` pointe vers la page d'index du filing — pas le contenu (qui vit dans Exhibit 99.1 attaché). DoD ad-hoc 30/05 : 3 8-K passées à V2 avec contenu de l'URL stockée → toutes prob=0.500 watch ev=none. **V2 a raison** : *"boilerplate header/cover page only, no actual earnings data"*. Effort A3 révisé : (1) extracteur d'exhibits 1-2h, (2) wire forward 1h. Décision masquée backfill/forward déjà tranchée : **C forward-only + DoD ad-hoc** (ledger pur). DoD final pas encore atteint — exhibits manquants. Session fraîche.
