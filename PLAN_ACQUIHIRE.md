...
- **A3 reste** : wire forward `8k_to_signal` job (~1h) qui appelle `extract_filing_content` quand une nouvelle 8-K arrive dans `filings_8k_log` et insère dans `signals`. Session fraîche pour décisions schéma/pipeline (signal_type à attribuer, source dédiée, dedup logic).
