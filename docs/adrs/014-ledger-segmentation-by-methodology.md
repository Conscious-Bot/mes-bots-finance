...
| **Substance accounting LLM** (scorer inputs, accounting opérationnel, LLM-loop audit) | `substance_predictions_filter()` -- exclut v0 + familles non-LLM (rule_v1_shadow, rule_v1_fallback) | `intelligence/base_rates.py`, `intelligence/outcome_context.py`, `intelligence/v2_vigilance.py`, `intelligence/portfolio_grade.py`, `intelligence/thesis_track_record.py`, `intelligence/morning_brief.py`, `dashboard/render.py:_loop` |
| **User-facing lookup** (recherche prédiction par ticker, audit explicite tout-types) | `methodology_version != 'v0'` direct (exclut seulement quarantine, inclut shadow/fallback avec provenance affichée) | `bot/handlers/prediction_why.py` |
...
**Future retirement (V2 → V3 someday)** : quand V2 sera déprécié vers V3, refaire cet exercice : ajouter V2 dans `CANONICAL_METHODOLOGY_EXCLUSIONS` (V2 sort du headline public, V3 prend la place), créer un V2-archive report explicite (J-day style), garder `substance_predictions_filter()` inchangé (V2 reste LLM-substance + V3 s'y ajoute automatiquement, pas de touch). Si V3 introduit une nouvelle famille non-LLM (ex: `rule_v2_shadow`), ajouter dans `SUBSTANCE_METHODOLOGY_EXCLUSIONS` symétrique à `rule_v1_*`.

**Relation canonical ⊂ substance** (testée structurellement, `test_substance_tier.py::test_canonical_strict_subset_of_substance`) : toute ligne forward-headline est aussi substance accounting. L'inverse pas : v1 archive est substance (alimente base_rates, outcome_context) mais pas headline. Cette asymétrie est la définition même du headline-vs-substance split.
