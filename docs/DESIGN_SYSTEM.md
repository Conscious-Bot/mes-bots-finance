...
15. **Switch mode jour/nuit : scroll visibility canonique, esthétique soft.** Seule la position est canonique inflexible : `position:fixed !important; left:var(--s25); bottom:var(--s25); z-index:90` — invariant quel que soit le scroll, l'overflow sidebar, le viewport ou les responsive breakpoints. Le **scroll suit la vision user**. L'esthétique du bouton reste **soft/discrète** (border 1px `--line`, color `--steel`, bg `--bg`) — pas de contraste agressif, ne détonne pas avec la sobriété générale.
