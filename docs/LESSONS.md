...
**Le mécanisme (à conserver)** : `target_partial` et `target_full` sont **figées à la pose de thèse** ; `cost_native = avg_cost_eur / fx_now` **roule** sur **deux** axes — (a) les **renforcements** (avg_cost_eur change) et (b) le **FX** (fx_now bouge, surtout sur tickers non-EUR comme KRW/JPY). Quand le cost rattrape ou dépasse une cible figée, la cible devient un mensonge silencieux (un "partial" à exécuter qui est en fait une coupe à perte vs cost). Le partial est juste le **canari** — la cible la plus basse, touchée en premier. Le full SK Hynix à −0.7% est le **même** mécanisme à un niveau plus extrême. Doctrine au niveau "cible-figée-rattrapée-par-le-cost", pas au niveau des partiels seuls.
