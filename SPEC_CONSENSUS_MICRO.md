...
> La seconde projection du cornerstone (cf `SPEC_CORNERSTONE.md`). Hérite de `CALIBRATION_DOCTRINE.md`, `PLAN_REFONTE_ALERTES.md`. **Point central : la projection micro a une situation statistique OPPOSÉE à la macro — et donc une stratégie de calibration opposée.** Doc reçu 08/06, archivé pour build futur post-cornerstone macro.
...
- Le consensus est là où le projet **peut avoir une validation statistique FORTE**. Contrairement à la crisis gauge, tu as la puissance.
- Les inputs micro sont des facteurs documentés (positionnement/crowding, earnings-revision momentum, short interest) → signes et priors empruntés à la recherche, pas devinés.
- La fragilité (cycle × consensus) hérite de la confiance la PLUS FAIBLE des deux — le marginal micro est fort, le conditionnement-cycle est un prior faible (N-starved sur la phase). La présentation doit refléter cette différence de confiance, jamais la masquer.
