...
def env(key: str, default=None, cast=str):
    ...
    if v is None:
        return None
    try:
        return cast(v) if cast != bool else str(v).lower() in ("true", "1", "yes")
    except Exception:
        return default

def capital() -> float: return env("CAPITAL", 10000, float)
def paper_only() -> bool: return env("PAPER_ONLY", "true", bool)
def min_conviction() -> int: return env("MIN_CONVICTION", 3, int)
def telegram_chat_id() -> int: return env("TELEGRAM_CHAT_ID", 0, int)
def telegram_token() -> str: return env("TELEGRAM_BOT_TOKEN")
