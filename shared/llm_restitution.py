"""Cure P2 audit (3) reste whitelist 12/06/2026 — degraded restitution contract
dans la bonne couche (shared/, pas dashboard/).

Helpers de format string pour les markers SYNTHESIZED-down (LLM unavailable
+ rule fallback provenance). Pure logique de formatage (constante + ~5 lignes
de string formatting). Zéro rendu HTML/CSS, zéro dépendance dashboard.

Anti-pattern reconnu : `intelligence/analyze.py:534` importait
`from dashboard.restitution import format_llm_unavailable_marker` (couplage
intelligence/ → dashboard/, dernière entrée du whitelist legacy). Déplacement
ici tue le couplage et **vide complètement** `_INTELLIGENCE_LEGACY_WHITELIST`.

dashboard/restitution.py devient un alias rétro-compat (re-exports depuis ici)
pour ne pas casser les callers internes au dashboard ou les tests existants.
