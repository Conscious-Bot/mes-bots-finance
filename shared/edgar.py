...
def format_insider_summary(activity, top_n=15):
    ...
    for a in sorted_act[:top_n]:
        ...
        owner = (a['owner'] or '?')[:22]
        role = (a['role'] or '?')[:20]
        lines.append(f"  {a['filing_date']} {cl:5} {owner:22} [{role:20}] "
...
def get_insider_brief(ticker, days=90, ttl_hours=INSIDER_CACHE_TTL_HOURS):
    ...
    cache_file = _os.path.join(CACHE_DIR, f'insiders_{ticker.upper()}.json')
    ...
    if not activity:
        brief = {'ticker': ticker, 'net_m': 0.0, 'buys_m': 0.0, 'sells_m': 0.0,
                 'n_buys': 0, 'n_sells': 0, 'n_big_sales': 0}
    else:
        purchases = [a for a in activity if a['code'] == 'P']
        sales = [a for a in activity if a['code'] == 'S']
        total_p = sum(a['value'] for a in purchases)
...
def format_insider_context_for_prompt(briefs):
    ...
    for b in active[:15]:
        ...
        net = b['net_m']
        sign = '+' if net > 0 else ''
        lines.append(f"  {tk}: net {sign}{net:.1f}M (P:${b['buys_m']:.1f}M / S:${b['sells_m']:.1f}M, {b['n_buys']}P/{b['n_sells']}S)")
    lines.append("")
...
def _role_weight(role):
    r = (role or "").lower()
    if any(x in r for x in ("ceo", "chief executive")): return 5
    if any(x in r for x in ("cfo", "chief financial")): return 4
    if any(x in r for x in ("coo", "cto", "cio", "chief")): return 3
    if "director" in r: return 2
    if "10%" in r or "owner" in r: return 1
    return 1
...
def _classify_buy_cluster(buyers):
    ...
    if n >= 5 and total_m >= 5.0: return "strong"
    if n >= 3 and max_role >= 4 and total_m >= 5.0: return "strong"
    if n >= 3 and total_m >= 1.0: return "moderate"
    if n >= 3: return "weak"
    return "none"
...
def get_insider_cluster(ticker, days=14):
    ...
    for t in activity or []:
        ...
        try:
            td = datetime.strptime(td_str[:10], "%Y-%m-%d")
        ...
        if not owner or value_m == 0: continue
