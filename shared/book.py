...
def _load_target() -> dict:
    """Lit la cible book depuis config/target_allocation.yaml (canonical).

    Returns dict en forme legacy (keys: positions, phantoms_in_db_not_in_target,
    _meta). YAML valide via Pydantic TargetAllocationConfig au load.

    Validation echec -> log.warning + dict vide (fail-safe, ne plante pas
    le bot mais signale en log).
    """
    global _TARGET_CACHE
    if _TARGET_CACHE is not None:
        return _TARGET_CACHE

    if not _TARGET_YAML_PATH.exists():
        import logging
        logging.getLogger(__name__).warning(
            f"target_allocation.yaml absent : {_TARGET_YAML_PATH}"
        )
